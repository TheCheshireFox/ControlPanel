...
static void lvgl_init_logging() {
    static constexpr char TAG[] = "LVGL";
    ...
    xTaskCreatePinnedToCore(+[](void*)
    {
        ...
        while(true)
        {
            ...
            {
                switch (rec.level) {
                    case LV_LOG_LEVEL_TRACE: ESP_LOGD(TAG, "%s", rec.buf); break;
                    case LV_LOG_LEVEL_INFO: ESP_LOGI(TAG, "%s", rec.buf); break;
                    case LV_LOG_LEVEL_WARN: ESP_LOGW(TAG, "%s", rec.buf); break;
                    case LV_LOG_LEVEL_ERROR: ESP_LOGE(TAG, "%s", rec.buf); break;
                    case LV_LOG_LEVEL_USER: ESP_LOGI(TAG, "%s", rec.buf); break;
                    default: ESP_LOGI(TAG, "%s", rec.buf); break;
                }
            }
        }
